...
export default {
  ...
  methods: {
    ...
    initializeMap: function() {
      console.log("new val>>>>", this.mapPoints[0].latitude)
      this.mapOptions = {
        zoom: 10,
    ...
    animationLoop: function(progressPoint) {
      ...
      this.myInterval = setInterval(() => {
        ...
        if (this.step > this.stepSize) {
          ...
          var progress = google.maps.geometry.spherical.interpolate(
            this.startPoint,
            this.endPoint,
            this.step / this.stepSize
          );
          this.myPath.setPath([this.startPoint, progress]);
        }
      }, this.stepTime);
    },
    ...
    dynMapData(data) {
      ...
      if (this.stopFlag) {
        ...
        this.pathAnimation(lastMapIndex);
      }
      this.getDataFlag = 1;
    },
